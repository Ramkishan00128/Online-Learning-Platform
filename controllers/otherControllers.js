...
export const contact = catchAsyncError(async (req, res, next) => {
  ...
  if (!name || !email || !message)
    return next(new ErrorHandler("All field are mandotory", 400));
...
export const courseRequest = catchAsyncError(async (req, res, next) => {
  ...
  await sendEmail(to, subject, course);
...
export const getDashboardStats = catchAsyncError(async (req, res, next) => {
  const stats = await Stats.find({}).sort({ createdAt: "desc" }).limit(12); //desc means decrement, nodecron will create new stat each month
  const statsData = [];
